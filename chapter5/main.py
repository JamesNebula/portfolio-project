...
api_description = """
This API provides read-only access to info from the SportsWo(SWC) Fantasy Football API.
The endpoints are grouped into the following categories:

## Analytics
Get information about the health of the API and counts of leagues and players.

## Player
You can get a list of NFL players, or search for an individual player_id.

## Scoring
You can get a list of NFL player performances, including thethey scored using SWC league scoring.

## Membership
Get information about all the SWC fantasy football leagues.
"""
...
@app.get(
        "/",
        summary="Check to see if the SWC fanstasy football API is running",
...
@app.get(
        ...
        summary="Get all the SWC players that meet all the parameters sent with your request",
        ...
        response_description="A list of NFL players that are in SWC fantasy football. They don't need to be on a team.",
...
def read_players(
    ...
    minimum_last_changed_date: date = Query(
        None, description="The minimun data of change that you want to return records. Exclude any records changed before this."
    ), 
...
@app.get(
        ...
        description="If you have an SWC player ID of a player from another API call such as v0/get_players, you can call this API using the player ID",
...
@app.get(
        ...
        description="""Use this endpoint to get lists of weekly performances by players in the SWC. You use the skip and limit to perform pagination of the API. Don't use the Performance ID for counting logic, because that is an internal ID and is not guaranteed to be sequential.""",
...
def read_performances(
    skip: int = Query(
        0, description="The number of items to skip at the beginning of that API call."
    ), 
    limit: int = Query(
        100, description="The number of records to return after the skipped records."
    ), 
    minimum_last_changed_date: date = Query(
        None, description="The minimum data of change that you want to return records. Exclude any records changed before this."
    ), 
    db: Session = Depends(get_db)
):
